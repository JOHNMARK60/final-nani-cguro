...
<main class="min-h-screen bg-slate-100 px-4 py-8 print:bg-white">
    <div class="mx-auto max-w-4xl bg-white p-8 shadow-soft print:shadow-none">
        <div class="grid gap-6 sm:grid-cols-[1fr_240px]">
            <div class="font-sans text-6xl font-bold tracking-wide text-[#7a5a00]">RECEIPT</div>
        ...
        <section class="mt-8">
            <div class="bg-[#B8860B] px-2 py-1 font-sans text-2xl text-white">CUSTOMER INFORMATION</div>
            <div class="space-y-4 border-x-2 border-[#B8860B] px-1 py-4 font-sans text-xl">
                <div>NAME: <span class="inline-block min-w-[80%] border-b border-slate-900 px-3"><?= e($user['fullname'] ?? 'Parish Member') ?></span></div>
                <div>ADDRESS: <span class="inline-block min-w-[75%] border-b border-slate-900 px-3"><?= e($user['address'] ?? '') ?></span></div>
        ...
        <section>
            <div class="bg-[#B8860B] px-2 py-1 font-sans text-2xl text-white">BILL OF MATERIALS OR SERVICES</div>
            <table class="w-full border-2 border-[#B8860B] font-sans text-xl">
                <thead>
                    <tr class="border-b-2 border-[#B8860B] text-left">
                        <th class="border-r-2 border-[#B8860B] p-2">DESCRIPTION</th>
                        <th class="border-r-2 border-[#B8860B] p-2 text-center">QTY</th>
                        <th class="border-r-2 border-[#B8860B] p-2 text-center">UNIT COST</th>
                        <th class="p-2 text-center">AMOUNT</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="h-14 border-b-2 border-[#B8860B]">
                        <td class="border-r-2 border-[#B8860B] p-2"><?= e($payment['description']) ?></td>
                        <td class="border-r-2 border-[#B8860B] p-2 text-center">1</td>
                        <td class="border-r-2 border-[#B8860B] p-2 text-right"><?= e(peso($payment['original_amount'] ?: $payment['amount'])) ?></td>
                        <td class="p-2 text-right"><?= e(peso($payment['amount'])) ?></td>
                    </tr>
                    <?php for ($i = 0; $i < 7; $i++): ?>
                        <tr class="h-12 border-b-2 border-[#B8860B]"><td class="border-r-2 border-[#B8860B]"></td><td class="border-r-2 border-[#B8860B]"></td><td class="border-r-2 border-[#B8860B]"></td><td></td></tr>
                    <?php endfor; ?>
                </tbody>
            </table>
        </section>
        ...
        <section class="mt-8 grid gap-6 sm:grid-cols-[1fr_260px]">
            <div class="min-h-32 border-2 border-[#B8860B] p-2 font-sans text-xl">
                COMMENTS:<br>
            ...
            <div class="space-y-3 font-sans text-xl">
                <div class="flex justify-between gap-4">SUBTOTAL: <span class="min-w-32 border-b border-slate-900 text-right"><?= e(peso($payment['original_amount'] ?: $payment['amount'])) ?></span></div>
                <div class="flex justify-between gap-4">DISCOUNT: <span class="min-w-32 border-b border-slate-900 text-right">-<?= e(peso($payment['discount_amount'] ?? 0)) ?></span></div>
        ...
        <div class="mt-8 border-2 border-[#B8860B] p-3 text-center font-sans text-xl">Thank you for supporting the parish!</div>
